...
class LightSource {
 public:
  ...
  inline LightSource(Vec3f position, Vec3f color, Vec3f direction,
                     float intensity, float sideLength)
      : uniform(-sideLength, sideLength) {
    m_position = position;
    m_color = color;
    m_intensity = intensity;
  ...
  inline Vec3f randAreaPosition() {
    return m_position + (uniform(gen) * m_vertical) +
           (uniform(gen) * m_horizontal);
  }
