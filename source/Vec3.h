...
class Vec3 {
 public:
  ...
  inline Vec3(const Vec3& v) { init(v[0], v[1], v[2]); }

  ~Vec3() {}

  inline T& operator[](int Index) { return (m_p[Index]); };

  inline const T& operator[](int Index) const { return (m_p[Index]); };
  ...
  inline T squaredLength() const { return (dot(*this, *this)); };

  inline T length() const { return (T)sqrt(squaredLength()); };

  /// Return length after normalization
  inline T normalize() {
    T l = length();
    if (l == T(0.0)) return T(0.0);
    T invL = T(1.0) / l;
    m_p[0] *= invL;
    m_p[1] *= invL;
    m_p[2] *= invL;
