...
class PhotonMap {
    ...
    public:
        ...
        {
            if (numOfPhotons > 0 ) {
                cout << "Constructing a photon map with " << numOfPhotons << " photons" << endl;
                m_rayTracer = rayTracer;
                float lightPdf = 1.f / scene.lightsources().size();
                int photonsPerLS = (int)(numOfPhotons * lightPdf);
                cout << "Emitting " << photonsPerLS << " photons per light source" << endl;
                ...
                for (LightSource lightSource : scene.lightsources()) {
                    ...
                    for (int i = 0; i < photonsPerLS; i++){
                        ...
                        Vec3f startPosition = lightSource.randAreaPosition(),
                              startDirection = m_rayTracer.hsphereUniformSample(lsNormal, M_PI / 2.f);
                        
                        //  Assign higher probabilities to photons with a direction close to normal
        ...
        inline int calculatePhotonPath (const Scene & scene, Ray ray, Particle photon, int depth, bool exit)
        {
            if (exit){
            ...
            // pdf is the non-negative cos distance between perfect reflection and a random direction
